...
class Level implements ChunkManager, Metadatable{
	...
	public function __construct(Server $server, string $name, string $path, string $provider) {
        ...
        $this->dimension = self::DIMENSION_NORMAL;

		if ($this->server->netherEnabled and $this->server->netherName == $this->folderName)
			$this->dimension = self::DIMENSION_NETHER;
		elseif ($this->server->enderEnabled and $this->server->enderName == $this->folderName)
            $this->dimension = self::DIMENSION_END;

		if ($this->server->weatherEnabled and $this->dimension == self::DIMENSION_NORMAL) {
    ...
    public function setSleepTicks(int $ticks){
    ...
    public function dropExperience(Vector3 $pos, int $amount) : array{
        ...
        foreach(ExperienceOrb::splitIntoOrbSizes($amount) as $split){
            ...
            $orb = Entity::createEntity("ExperienceOrb", $this, $nbt);
            if($orb === null){
                continue;
            }
     ...
     */
    public function getNearestEntity(Vector3 $pos, float $maxDistance, string $entityType = Entity::class, bool $includeDead = false){
	 ...
	 * @deprecated Please use Level->dropExperience
	 * @param Vector3 $pos
	 * @param int $exp
	 * @return ExperienceOrb[]
	 */
	public function spawnXPOrb(Vector3 $pos, int $exp = 1) {
		return $this->dropExperience($pos, $exp);
	}
    ...
    public function isUpdateScheduled(Vector3 $pos, Block $block){
        return isset($this->scheduledBlockUpdateQueueIndex[$index = Level::blockHash($pos->x, $pos->y, $pos->z)]);
    }
